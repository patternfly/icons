...
const getSVG = (name: string, { license, height, width, path, xOffset = 0, yOffset = 0 }: IconSpec) =>
  `<!-- © ${license} licensed --><svg xmlns="http://www.w3.org/2000/svg" data-icon-name="${name}" height="${height}" width="${width}" viewBox="${[xOffset, yOffset, width, height].join(' ')}"><path d="${path}" /></svg>`
